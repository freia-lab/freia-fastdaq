...
x=[0.0;0.0];  xp=[0.0;0.0];
P=eye(3);                                % initial value of P
qhat=zeros(3,1);                    % initial parameter estimate  %data=zeros(Niter,3);      % storage for later plotting
...
if test    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 Areal=eye(2)+F0;       
 Breal=R*omega12*dt*eye(2);  xp=sigm*randn(2,1);
 if dt == 1e-7
